// Decompiled by Jad v1.5.8e2. Copyright 2001 Pavel Kouznetsov.
// Jad home page: http://kpdus.tripod.com/jad.html
// Decompiler options: packimports(3) fieldsfirst ansi space 
// Source File Name:   ActionMappingDAOFactory.java

package com.shenghao.arch.actionmapping;


// Referenced classes of package com.shenghao.arch.actionmapping:
//			ActionMappingDAOImpl, ActionMappingDAO

public class ActionMappingDAOFactory
{

	public ActionMappingDAOFactory()
	{
	}

	public static ActionMappingDAO getDAO()
	{
		ActionMappingDAOImpl actionmappingdaoimpl = new ActionMappingDAOImpl();
		return actionmappingdaoimpl;
	}
}
