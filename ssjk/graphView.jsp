...
<%
	...
	while (crs_1.next()) {
		...
		pageFreshTime = client.core.util.Tools.delNull(crs_1
				.getString("pageFreshTime"), "5");

	}
	System.out.println("parameteName========================="
			+ parameteName);
	System.out.println("configYPerValue========================="
			+ configYPerValue);
	System.out.println("configYMinValue========================="
			+ configYMinValue);
	System.out.println("configYValueCount========================="
			+ configYValueCount);
	System.out.println("pageFreshTime========================="
			+ pageFreshTime);
%>
...
<%!public static String getHMS(String stime) {
	...
	vc.setPointsValue(xValueArr,yValueArr,sValueArr);	
					...
					<div style="padding-left: 5px;">
						自动刷新频率
						<input name="autoFlash_frq" type="text" value='<%=pageFreshTime%>'
							style="width: 30px;" />
						秒
					</div>
		</div>
...
<script>
	function autoFresh() {
		location = "MainServlet_sys?fid=FCL00.00.01.12&StatusName=
<%=StatusName%>&StatusCol=<%=StatusCol%>&tp_carname=<%=tp_carname%>&Tnum=<%=Tnum%>&parameteName=<%=StatusCol%>&v_pl=<%=pageFreshTime%>";
}
