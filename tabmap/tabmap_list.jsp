...
function confirmButton(operationMode, tr_row,id_value){
	...
	submitframe.location = "MainServlet_sys?fid=UII99.99.99.02&id="+id_value;
	location.reload();
	return ok;
}
</script> 
...
<div id="man_zone" style="padding-top:5px; ">
<table class=table style="BORDER-COLLAPSE: collapse" borderColor=#dedfd7 cellSpacing=0 cellPadding=3 width="100%" align=center bgColor=#ffffff border=1 >
	<tr ><td bgColor=#f0f0f0 height=25  align="center" class="text2"><font size="+1">列名字典</font></td></tr>
</table>
<div id="listdiv" style="width:100%; overflow-x:scroll; height:415px;">
			...
			<form name="frmList" target="submitframe"></form>
				...
				<%
					for(int jk=0; jk<al_colname.size(); jk++){
						%>
						<td align="left" width="13%" class="center_title_1" nowrap="nowrap">&nbsp;&nbsp;<%=(String)al_colnamec.get(jk)%></TD>
						<%
					}
				%>
				<%
				...
				%>
				<tr align="center">
				<td colspan="<%=al_colname.size()+1%>"><p align="center" class="text2">未查询到单位类型</td>
				</tr>
				<%
				}
				else
				...
				<tr bgcolor='FFFFFF' class='font_px12_menu' id="tr_<%=crs.getString("id")%>" onMouseOver="javascript:this.style.background= '#ECF2FC';" onMouseOut="javascript:this.style.background= '#FFFFFF';">
					...
					<td height="2" align="center" class="text3" nowrap="nowrap" id="tr_<%=i_row%>_edit2">
						<a href="#" onclick="modify('MainServlet_sys?fid=UII99.99.99.04&id=<%=crs.getString("id")%>&mode=before')"><font color="#053080">修改</font></a>
						<a href="#" onclick="return confirmButton('delete', <%=i_row%>, <%=crs.getString("id")%>);" ><font color="#ff0000" class="tdrecord" style="font-size:12px" >删除</font></a>
					</td>
					<%
					}
					%>
...
<table width='100%' border='0' align='center' cellpadding='1' cellspacing='1' bordercolor='#bdd7d6' bgcolor='#bdd7d6'  rules='none' style="font-size:12px;background-color:#fff;">
	<tr id='pagecon'>
		<td width='55%' scope='col' align='left' id='listTitle' >数据列表显示区(全部记录)</td>
...
<div >
<div style="float:left;"onclick="onquery()" id="onquery">【查询】</div>
<div style="float:left;"onclick="onmodify()"id="onmodify">【修改】</div>
<div style="float:left;"onclick="onclose()" id="onclose">【关闭】</div>
</div>
<iframe id="queryframe" name="queryframe" width="100%" height="300" src="" frameborder="NONE" style="display:none"></iframe>
<iframe id="modifyframe" name="modifyframe" width="100%" height="300" src="" frameborder="NONE" style="display:none"></iframe>
</body>
...
$('.font_px12_menu').contextMenu('myMenu1', {
  bindings: {
	'see': function(t) {
	  ...
	  show('MainServlet_sys?fid=UII00.00.03.05&id='+ t.id.split("_")[1])
	},
	'edit': function(t) {
	  //alert('Trigger was '+t.id+'\nAction was edit');
	  modify('MainServlet_sys?fid=UII99.99.99.04&mode=before&id='+ t.id.split("_")[1])
	},
	'save': function(t) {
	  alert('Trigger was '+t.id+'\nAction was Save');
	},
...
function modify(s)
  {
		document.getElementById("queryframe").style.display="none";
		document.getElementById("modifyframe").style.display="block";
		document.getElementById("listdiv").style.height="100";
		window.top.indexFrame.frames['manFrame'].modifyframe.location = s;

	}
function onquery()
  {
		document.getElementById("queryframe").style.display="block";
		document.getElementById("listdiv").style.height="100";
		document.getElementById("modifyframe").style.display="none";

		window.top.indexFrame.frames['manFrame'].queryframe.location = 'MainServlet_sys?fid=UII99.99.99.06&mode=before';
	}
/* function modify()
  {
		document.getElementById("modifyframe").style.display="block";
		document.getElementById("listdiv").style.height="100";
		document.getElementById("man_zone").style.height="150";
	  window.top.indexFrame.frames['manFrame'].modifyframe.location = s;

	} */
function onclose()
  {
		document.getElementById("listdiv").style.height="415";
		document.getElementById("man_zone").style.height="500";
