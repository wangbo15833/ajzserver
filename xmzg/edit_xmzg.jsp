...
<%@ page import="client.core.util.Tools" %>
...
<!-- <TABLE class=table style="BORDER-COLLAPSE: collapse" borderColor=#6595d6 cellSpacing=0 cellPadding=3 width="90%" align=center bgColor=#ffffff border=1>-->
	...
	<input type="hidden" name="fid" id="fid" value="UII00.04.03.04"><input type="hidden" name="id" id="id" value="<%=crs.getString("id")%>">
		<%
	int ik = 0;
...
<script language=javascript>
  ...
  if(document.getElementById("cont")!=null){
	...
	document.getElementById("cont").value=s;}
    formHead.target = "submitframe";
	document.getElementById("fid").value="UII00.04.03.03";
	formHead.submit();
	//alert("已提交");
  }
  function doClose()
...
<script language="javascript">
	function  checknow()
	{
		if(<%=flag1%>){
		alert("修改成功！");
		window.top.indexFrame.frames['manFrame'].location = 'MainServlet_sys?fid=UII00.04.03.01<%=Integer.parseInt(mypri)==4?"":filter_str%>';
		}
	}
	checknow();
</script>
