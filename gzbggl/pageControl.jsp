...
if(eventResponse.isSuccessFlag()){ 
	...
	if(eventResponse.getFunctionID().equals("FCL00.00.06.01")){ 
		if(Tools.nvl(request.getParameter("fileName")).equals("ba_dbom_master_id.jsp")){
	...
	else if(eventResponse.getFunctionID().equals("FCL00.00.06.04")){
		String listMode= Tools.nvl(request.getParameter("listMode")); 
		out.println(SysTrace.showMsg("修改成功！"));
		out.println("<script language=JavaScript>");
			%>
			parent.location = 'MainServlet_sys?fid=FCL00.00.06.02&listMode=<%=listMode%>';
			<%
		out.println("</script>");
	}
	
	else if(eventResponse.getFunctionID().equals("FCL00.00.06.05")){ 
		out.println(SysTrace.showMsg("删除成功！"));
		out.println("<script language=JavaScript>");
			%>
			//parent.location = 'MainServlet_sys?fid=FCL00.00.06.02';
			<%
		out.println("parent.location = 'MainServlet_sys?fid=FCL00.00.06.02';");
		out.println("</script>");
	}
